...
def main() -> int:
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Read cookies from a local browser (chrome, firefox, safari, edge, brave, …) "
             "for login-gated sources (Instagram, X, private/age-restricted videos).",
    )
    ap.add_argument(
        "--cookies",
        type=str,
    ...
    ap.add_argument(
        "--no-whisper",
        action="store_true",
        help="Disable local Whisper fallback. Report frames-only if no captions available.",
    )
    ap.add_argument(
        "--whisper",
        choices=["mlx", "openai-whisper"],
        default=None,
        help="Force a local Whisper engine. Default: prefer mlx-whisper, fall back to openai-whisper.",
    )
    args = ap.parse_args()
    ...
    if not transcript_segments and not args.no_whisper:
        backend, model = detect_backend(args.whisper)
        if backend:
            try:
            ...
            hint = (
                f"--whisper {args.whisper} was set but that engine isn't installed"
                if args.whisper else
                "no subtitles and no local whisper engine found"
            )
            setup_py = SCRIPT_DIR / "setup.py"
            print(
                f"[watch] {hint} — run `python3 {setup_py}` to enable local transcription",
                file=sys.stderr,
            )
